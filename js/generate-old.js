...
function buildBaseArgs(pjd) {
  ...
  sorted.forEach((ps, i) => {
    ...
    pesertaIndexed[`pangkat_peserta_${n}`] = pgw?.pangkat || '';
...
async function runGenerate(pjdId, selections) {
  ...
  for (const sel of selections) {
    ...
    if (!tmpl?.fileData) { errors.push(`${sel.label}: file tidak tersedia`); continue; }

    try {
      updateGenerateProgress(`Generating ${sel.label}...`);

      if (tmpl.isIterable) {
        // ── Kwitansi: loop per peserta ───────────────────
        const sorted = sortedPeserta(pjd);
        for (let i = 0; i < sorted.length; i++) {
          ...
          const blob = await generateDocx(tmpl.fileData, args);
          folder.file(`Kwitansi_${i+1}_${nama}.docx`, blob);
          count++;
        }
      } else if (tmpl.jenis === 'sppd') {
        // ── SPPD: generate 2 lembar ──────────────────────
  ...
  // Catat di history
  const gen = JSON.parse(localStorage.getItem('sppd_generated') || '[]');
  gen.unshift({ id: Date.now(), pjd_id: pjdId, nomor: pjd.nomor_surat, count, errors: errors.length, at: new Date().toISOString() });
  localStorage.setItem('sppd_generated', JSON.stringify(gen.slice(0, 50)));
...
function renderGeneratePage() {
  ...
  const genHistory = JSON.parse(localStorage.getItem('sppd_generated') || '[]').slice(0, 5);
